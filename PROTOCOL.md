...
- **床號不開放人工設定**：由財編向院方設備系統（maya）查詢後自動帶入，由伺服器寫入同一個欄位（見下方「床號自動帶入」）。管理 API 仍接受 `bed`（供自動帶入使用），但管理頁不提供這個輸入欄位——多一個人工來源只會多一份會過期的資料。查不到（未啟用、未登記財編、maya 上尚無位置）時床號維持空字串，儀表板顯示機台編號。
...
床號的真相來源是院方設備系統（maya）：醫療人員把呼吸器接上病人時，會在 maya 的機器上
更新該台的使用位置，伺服器據此把床號帶進裝置清冊。資料流是
**財編（管理員登記於清冊）→ 查 maya `getDeviceList` → 取該筆床號 → 寫入 `bed` → 廣播 `device_meta`**，
看板收到後即時換卡片標題並重新排序。

🚨 **只取床號與記錄時間；姓名與病歷號在解析當下就丟棄。** maya 的 `ON_POSITION` 字串同時
帶有病人姓名與病歷號，本系統一律不取用、不留存、不廣播、不寫 log——床號不是個資，
姓名與病歷號是（CLAUDE.md §2.2）。解析函式的回傳值裡根本沒有這兩個欄位，下游想留也留不到。
畫面上的病歷號一律來自床邊輸入的 `hello.patient`，與 maya 無關。

**何時查詢**（每台裝置各自判斷；`A` = 該台呼吸器連線建立的時刻）：

| 事件 | 動作 |
|---|---|
| 呼吸器連線建立（`status` 由非 connected 轉為 `connected`） | 記下此刻為 **A**，該台進入待查，每 `maya_poll_interval`（預設 30 秒）查一次 |
| 查到的記錄時間**晚於 A** | 這是本次接機後才更新的位置 → 採用床號並**停止查詢** |
| 查到的記錄時間**早於 A** | 還是上一位病人的舊資料 → 繼續查 |
| 呼吸器斷線（`status` 轉為其他狀態） | 停止查詢——沒有新病人要記錄 |
| Pi 離線 | **暫停**查詢，該台重新上線後接續（總時限仍從 A 起算）。刻意不是取消：Wi-Fi 抖一下就永久放棄查詢，床號會莫名其妙一直空著 |
| 自 A 起超過 `maya_max_duration`（預設 1 小時） | 放棄並記 log——不對院方正式系統無止境發請求 |

時間比較帶 `maya_time_tolerance`（預設 120 秒）寬容：**記錄時間 ≥ A − 容差**即視為新資料。
寬容方向刻意偏向「接受」有兩個理由：伺服器與 maya 主機的時鐘只靠 NTP 對齊；醫療人員也常
在接機前一兩分鐘就先在 maya 上更新。若反過來要求記錄時間必須晚於 A ＋ 容差，那些正確的
資料會被永遠拒絕、白白輪詢滿一小時。

**一次請求涵蓋全部待查裝置**：`getDeviceList` 回傳的是全院清單（數百筆），伺服器每輪只發
**一個**請求再逐台比對，不是每台各發一次。沒有任何「查得動」的待查裝置（全部已完成、
已離線、或都還沒登記財編）時，該輪完全不發請求。

**解析規則**（Tolerant Reader，CLAUDE.md §3.2）：以 `DEVICE_NO` 對應財編（比對前去空白、
不分大小寫）；床號與記錄時間取自 `ON_POSITION`，格式為
`床號：CCU18 姓名：… 病歷號：… 記錄時間:2026-08-04 08:56:06`（院方全形／半形冒號混用，
兩種都接受），記錄時間缺漏時退回同筆的 `RECORD_DATE`。同一財編有多筆時取記錄時間最新的
一筆。任何一筆解析失敗只跳過該筆，不影響其他裝置。**連線失敗或 HTTP 非 200 一律當成
「這輪沒查到」**：記 log、保留待查狀態、下輪再試，絕不影響儀表板既有顯示。

`maya_enabled` 預設 `false`——開發環境不對院方正式系統發請求，驗證一律用
`tools/fake_maya.py` 本地假伺服器。該 API 為院方既有設計的明文 HTTP，本系統只是讀取端。

### 單位（`ward`）——由床號推導，不是獨立欄位
...
| 床號 | `ward` |
|---|---|
| `MI01`／`CCU03`／`SI12`／`NI35`／`PI19`／`RCC21` | `MI`／`CCU`／`SI`／`NI`／`PI`／`RCC` |
| `IR01`／`BR21`／`BC02`（燒傷加護）／`HP13-1`（安寧多一個床位序號） | `IR`／`BR`／`BC`／`HP` |

⚠️ **刻意不寫死代碼清單。** 院方 `get_inPatient_wardList()` 列的 33 個單位**不完整**——燒傷加護（`BC`）就不在表上，但實際設備資料裡有（BC02）。寫死成 `^(MI|CCU|SI|NI|PI|RCC|IR|BR|HP)` 的話，`BC02` 會完全比對不到、該台機器在看板篩選時直接消失且無任何錯誤訊息；取字母前綴則是漏列或新增的單位都自動就對。

**一般病房**——格式 `{樓層2碼}{房間2碼}-{床位}`，樓層碼與病房代碼有固定換算：

| 系列 | 換算 | 例 |
|---|---|---|
| N08–N17 | 樓層碼 = 病房號 **+ 20** | `3520-1` → 樓層 35 → `N15`；`3236-1` → `N12` |
| R07、R09–R14（**無 R08**） | 樓層碼 = 病房號 | `0711-1` → `R07`；`1011-1` → `R10` |
| M08–M09、MP06–MP07 | 樓層碼 = 病房號 **+ 80** | `8811-1` → `M08`；`8611` → `MP06`（身心科一房一床，無床位序號） |
| 樓層碼不在表上 | — | `一般病房`（退路，見下） |

樓層碼認不得時退回 `一般病房` 而不是未指定：呼吸器有時會在一般病房使用，歸不了類的話那些機器會因為篩選不到而**整台從看板消失**——寧可粗一點，也不能讓機器不見。床號為空或格式認不得則回 `""`（未指定）。

⚠️ **G 院區（NSH9／NSH10／NSR11）不支援。** 判斷 G 院區必須看 `Zone` 欄位，而院方設備系統 `getDeviceList` 的回傳沒有這個欄位；且 `NSR11` 的床號 `A01-1` 會被字母前綴判成單位 `A`、`NSH10` 的 `1001-2` 與 `R10` 的 `1011-1` 都以樓層碼 `10` 開頭而無從區分。實測的呼吸器清冊裡沒有任何一台在 G 院區，因此暫不處理；日後若要納管，需要另一個帶 `Zone` 的資料來源，不是改這裡的規則。
...
| type | 時機 | 欄位 |
|---|---|---|
| `hello` | 連線後第一則 | `v` 協議版本、`device` 機台編號（hostname）、`patient` 病人代碼、`token` 存取權杖（見上）、`app_version` Pi 端軟體版本（選填，見「裝置連線時刻與軟體版本」）、`ts` |
...
管理頁需要回答兩個問題：**這台連上多久了**、**它在跑哪一版程式**。前者伺服器自己知道，後者只有 Pi 端知道，因此由 `hello` 帶上來。

| 欄位 | 來源 | 說明 |
|---|---|---|
| `connected_at` | 伺服器產生 | 本次連線建立（收到合法 `hello`）的伺服器時刻，Unix epoch 秒。**重連即更新**——它是「本次連線」的起點，不是裝置首次出現的時間 |
| `app_version` | Pi 端 `hello` 帶入 | Pi 專案的 `respiramark.__version__`。舊版 Pi 不送此欄 → 伺服器一律視為空字串，管理頁顯示 `—` |

- `connected_at` 只在 `hello` 時設定；watchdog 誤判離線後資料又進來而回復上線的情況**不重設**（TCP 連線從未中斷，重設會讓連線時長歸零而失真）。
- 伺服器只廣播這個**原始時刻**，「已連線 3 天 4 小時」這種格式化一律由前端做（見 CLAUDE.md §5 前後端分工）。
- ⚠️ 前端以「瀏覽器本機時間 − `connected_at`」計算時長，因此**伺服器與看板電腦的時鐘需同步**（NTP，見 IMPROVEMENT_PLAN.md W-206）；時鐘偏移多少，顯示的連線時長就偏多少。
